...
:: Direct call: run.bat install / wizard / validate / update / update-project / new-project / agent / prd
set "CMD=%~1"
shift
...
echo   5) prd            generate/update PRD.md
echo.
echo  --- UPDATE ---
echo   6) update         update ag_ai from GitHub
echo   7) update-project update agents in a project (keep context)
echo.
echo  --- WORK ---
echo   8) agent          launch OpenCode with agent menu
echo.
...
if "!CHOICE!"=="5" ( set /p PROJ=  Project path: & python "%AG_AI_DIR%\prd.py" "!PROJ!" --regenerate & pause & goto :eof )
if "!CHOICE!"=="6" ( cd /d "%AG_AI_DIR%" & git fetch origin & git reset --hard origin/main & echo Done. & pause & goto :eof )
if "!CHOICE!"=="7" ( set /p PROJ=  Project path: & python "%AG_AI_DIR%\setup_ai.py" "!PROJ!" --update-agents & pause & goto :eof )
if "!CHOICE!"=="8" ( call "%AG_AI_DIR%\agent.bat" & goto :eof )
