...
def fix_agent_colors(agents_dir):
    ...
    COLOR_MAP = {
        ...
        'metallic-blue': '#4a7fa5', 'neon-cyan': '#00ffff', 'neon-green': '#39ff14',
    }
    ...
    for f in Path(agents_dir).glob('*.md'):
        ...
        def replacer(m):
            ...
            return f'{m.group(1)}"{COLOR_MAP.get(raw.lower(), "#6366f1")}"'
        new_txt = COLOR_LINE.sub(replacer, txt)
        if new_txt != txt:
            f.write_text(new_txt, encoding='utf-8')
            fixed += 1
    if fixed:
        ok(f"Fixed {fixed} invalid color fields in agency agents")
